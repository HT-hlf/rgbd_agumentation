...
# @Software   : PyCharmkkkkkk'kk'k'k'kk'kk'k'k'kkkkkkkkkkkkkkkkkkkk'kk'k'kk'k'kk'k'kkk'kkkk
#无太多修改，较上版
import os
import cv2
import random
...
raw_root_path=r'G:\roadway_collect_dataset\recordData_process_annotation'
dst_root_path=r'G:\lab_collect_dataset\recordData_process_annotation_light_daytime - for_aug-negative_roadway_useful\1'
...
for raw_root_element in raw_root_list:
    ...
    dst_rgb_path_1 = os.path.join(dst_root_path, 'rgb')
    dst_depth_path_1 = os.path.join(dst_root_path, 'depth')
    dst_anno_path_1 = os.path.join(dst_root_path, 'annotation')
    creat_dir(dst_rgb_path_1)
    creat_dir(dst_depth_path_1)
    creat_dir(dst_anno_path_1)
    ...
    for i in range(len(rgb_image_list_1)):
        ...
        annotation_1_mid=random_rgb_1.rstrip('jpg') + 'xml'
        annotation_1 = anno_path_1 + '/' + annotation_1_mid
        ...
        while (True):
            ...
            cv2.imshow('ht_thre_1', depth_image_data_1_person_threshold_three_chan)
            ...
            if waitkey_count == 27:
                ...
                # print('j:有用')
                print('有用')
                f_dst = os.path.join(dst_rgb_path_1, random_rgb_1)
                shutil.move(rgb_image_path_1, f_dst)
                f_dst = os.path.join(dst_depth_path_1, random_rgb_1)
                shutil.move(depth_image_path_1, f_dst)
                f_dst = os.path.join(dst_anno_path_1, annotation_1_mid)
                shutil.move(annotation_1, f_dst)
                # cv2.imwrite()
                break
